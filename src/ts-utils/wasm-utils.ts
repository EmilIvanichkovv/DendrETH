...
export function writeMemoryArr(memory: MemorySlice, bytes: Array<number>): void {
  readMemory(memory).set(bytes, 0);
}

export function marshalArrayToWasm<T>(
  { memory, allocMemory }: WasmModuleMemoryInterface,
  array: Array<number>,
): MemorySlice {
  const startOffset = allocMemory(array.length);
  const slice: MemorySlice = {
    memory,
    startOffset,
    length: array.length,
  };
  writeMemoryArr(slice, array);
  return slice;
}
